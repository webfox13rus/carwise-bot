...
from handlers.monthly_reports import router as monthly_reports_router  # <-- добавили
from handlers.seasonal import router as seasonal_router  # если есть такой файл

# Импорты функций для планировщика
from handlers.monthly_reports import send_monthly_reports
from handlers.seasonal import send_seasonal_reminders  # если функция есть
...
async def main():
    ...
    dp.include_router(monthly_reports_router)   # <-- добавили
    dp.include_router(seasonal_router)          # если есть

    # Удаляем вебхук (если был установлен)
    await bot.delete_webhook(drop_pending_updates=True)
    
    # Настройка планировщика (сначала создаём, потом добавляем задачи)
    ...
    scheduler.add_job(send_monthly_reports, 'cron', hour=10, minute=0, args=(bot,))  # задача для ежемесячной статистики
    scheduler.start()
