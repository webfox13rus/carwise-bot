...
async def check_insurances(bot: Bot):
    ...
    try:
        with SessionLocal() as db:
            ...
            # Загружаем страховки, которые истекают в ближайшие 7 дней
            ...
            # Аналогично для 3 дней и просроченных...
            # (можно добавить, но для краткости опустим)
...
async def send_monthly_reports(bot: Bot):
    ...
    try:
        # Здесь должна быть логика отправки отчётов
        # (можно скопировать из вашего monthly_reports.py)
        pass
    except Exception as e:
