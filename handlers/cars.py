...
async def year_entered(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "Введите название или прозвище автомобиля (например, 'Моя ласточка') или отправьте /skip, чтобы пропустить:",
...
async def name_entered(message: types.Message, state: FSMContext):
    ...
    await state.set_state(CarStates.waiting_for_vin)
    await message.answer(
        "Введите VIN автомобиля (17 символов) или отправьте /skip, чтобы пропустить:",
        reply_markup=get_cancel_keyboard()
    )

@router.message(CarStates.waiting_for_vin)
async def vin_entered(message: types.Message, state: FSMContext):
    if message.text != "/skip":
        vin = message.text.strip().upper()
        if len(vin) != 17:
            await message.answer("❌ VIN должен содержать ровно 17 символов. Попробуйте ещё раз или /skip.")
            return
